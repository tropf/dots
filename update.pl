...
for $conf_entry (@conf) {
    ...
    # the keys
    # inc, rc, target, comment
    # always exist together: if one exists, all exist
    # -> check that
    
    if (exists $current_conf{"inc"} &&
        exists $current_conf{"rc"} &&
        exists $current_conf{"target"} &&
        exists $current_conf{"comment"}) {
        # all files exist, do file checks
    ...
    # two options to get execution to this point
    # a) script injection successfull
    # b) no script to inject, there is only the update script
